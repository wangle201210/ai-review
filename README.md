...
仓库内同时包含它依赖的
[`nova-victorialogs-query`](skills/nova-victorialogs-query/SKILL.md)、
...
Lark 会话映射键为 `<chat_id>:<thread_root>`。`thread_root` 优先使用 Lark 的
`root_id`，没有时依次回退到 `parent_id` 和当前消息 ID。当
`LARK__REQUIRE_REPLY=true`（当前服务器配置）时，顶层 `@机器人` 不会调用 Codex，
必须先回复一条告警或日志消息。
...
- 当 Codex 返回 `flagged for possible cybersecurity risk` 时，网关会在同一请求和
  总超时时限内自动恢复该 session 一次，并补充公司自有授权及防御性复现边界。
  如果恢复后仍被拦截，机器人会保存 session，并提示用户在原线程继续。
