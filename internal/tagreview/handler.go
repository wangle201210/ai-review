...
const WebhookPath = "/webhooks/gitlab/tag"

var (
	ErrQueueFull = errors.New("tag review queue is full")
...
func (r Review) DedupKey() string {
	sum := sha256.Sum256([]byte(fmt.Sprintf("%d\x00%s\x00%s", r.ProjectID, r.Tag, r.CommitSHA)))
	return "gitlab-tag:" + hex.EncodeToString(sum[:])
}
...
func NewHandler(enqueue EnqueueFunc, cfg Config) (*Handler, error) {
	...
	if allowedHost == "" || strings.ContainsAny(allowedHost, "/:@") {
		return nil, errors.New("GitLab tag review allowed host is invalid")
	}
	allowedNamespace := strings.Trim(strings.TrimSpace(cfg.AllowedNamespace), "/")
	if !validProjectPath(allowedNamespace) {
		return nil, errors.New("GitLab tag review allowed namespace is invalid")
	}
	if cfg.MaxRequestBytes < 1 {
		return nil, errors.New("GitLab tag review max request bytes must be greater than zero")
	}
	if cfg.Logger == nil {
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Header.Get("X-Gitlab-Event") != "Tag Push Hook" {
		writeJSON(w, http.StatusBadRequest, response{Status: "rejected", Reason: "not_a_tag_push_hook"})
		return
	}
	...
	if isZeroSHA(payload.After) || payload.CheckoutSHA == nil {
		writeJSON(w, http.StatusOK, response{Status: "ignored", Reason: "tag_deleted"})
		return
	}
	...
	if err != nil {
		h.logger.Printf("[gitlab-tag-review] enqueue failed project=%q tag=%q: %v", review.ProjectPath, review.Tag, err)
	...
	h.logger.Printf("[gitlab-tag-review] webhook %s project=%q tag=%q commit=%s", status, review.ProjectPath, review.Tag, review.CommitSHA)
...
func (h *Handler) decode(w http.ResponseWriter, r *http.Request) (tagPushPayload, error) {
	body := http.MaxBytesReader(w, r.Body, h.maxRequestBytes)
	defer body.Close()
	decoder := json.NewDecoder(body)
	var payload tagPushPayload
	if err := decoder.Decode(&payload); err != nil {
		return tagPushPayload{}, fmt.Errorf("invalid_json: %w", err)
	}
	var extra json.RawMessage
	if err := decoder.Decode(&extra); !errors.Is(err, io.EOF) {
		return tagPushPayload{}, errors.New("request_must_contain_one_json_object")
	}
	if payload.ObjectKind != "tag_push" || payload.EventName != "" && payload.EventName != "tag_push" {
		return tagPushPayload{}, errors.New("invalid_tag_push_payload")
	}
	return payload, nil
}

func (h *Handler) reviewFromPayload(payload tagPushPayload) (Review, error) {
	const tagPrefix = "refs/tags/"
	if !strings.HasPrefix(payload.Ref, tagPrefix) {
		return Review{}, errors.New("invalid_tag_ref")
	}
	tag := strings.TrimPrefix(payload.Ref, tagPrefix)
	if !validTag(tag) {
		return Review{}, errors.New("invalid_tag_name")
	}
	...
	if !shaRE.MatchString(commitSHA) || !shaRE.MatchString(strings.TrimSpace(payload.After)) {
		return Review{}, errors.New("invalid_checkout_sha")
	}
