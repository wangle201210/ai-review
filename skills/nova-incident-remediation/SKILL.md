...
- Respect the low-resource host constraints. Never run full repository test
  suites, `go test ./...`, full builds, Docker builds, broad linters, or other
  high-memory commands. Run only the narrowest relevant package or test.
...
4. Add or adjust a focused regression test when practical. Run only that test
   or its smallest package. If host constraints prevent execution, perform
   bounded static checks and say exactly what was not run.
